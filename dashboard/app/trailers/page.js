    ...
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, marginTop: 14 }}>
        ...
        <button
          ...
          title={syncDisabled && !pipelineSyncing ? 'No trailers ready to sync' : ''}
          style={{
            padding: '8px 16px',
            borderRadius: 6,
...
export default function ShowOverviewPage() {
  ...
  const loadLive = useCallback(async (showId, quiet) => {
    ...
    setLiveError(null);
    try {
      const r = await fetch(`/api/shows/${showId}/trailers-live`);
      ...
      setLiveError(e.message || 'Failed to load trailers');
      setLive(null);
    } finally {
  ...
  useEffect(() => {
    ...
    fetch(`/api/auto-pipeline/active/${selectedId}?kind=trailer`)
      .then(r => r.json())
      .then(data => {
        if (data?.run) {
  ...
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto' }}>
                ...
                onRunPipeline={async () => {
                  ...
                  try {
                    const res = await fetch('/api/auto-pipeline/start', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ showId: selectedId, kind: 'trailer' }),
                    });
                    const data = await res.json();
      ...
      {selectedId && liveLoading && !live && (
        <p style={{ color: '#888' }}>Loading trailers…</p>
      )}
        ...
        >
          ...
          >
            ...
            <span style={{ color: '#555', fontWeight: 400, marginLeft: 8 }}>
              {live.episodeCount} trailer{live.episodeCount !== 1 ? 's' : ''}
            </span>
            <S3NotificationBell
      ...
      {selectedId && !liveLoading && live && live.episodes.length === 0 && (
        <p style={{ color: '#888' }}>This show has no trailers in catalog data.</p>
      )}
    </div>
  );
}
...
function EpisodeActions({ row, onActionDone }) {
  ...
  // Trailers skip VTT generation — no thumbnails/subtitles for trailers.
  const handleGcpH264 = () => apiCall('gcp264', '/api/gcp', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
