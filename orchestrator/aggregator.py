...
def handler(event, context):
    ...
    try:
        ...
        if not results:
            # Raise so Step Functions marks this execution as FAILED, not silently succeeded
            raise RuntimeError(f"No research data found for episode_id={episode_id}")
        ...
        def find_winner(subset, threshold):
            ...
            if not subset:
                return None
            # Guard against None vmaf_score values
            valid = [r for r in subset if r.get("vmaf_score") is not None]
            if not valid:
                return None
            above = [r for r in valid if r["vmaf_score"] >= threshold]
            if above:
                winner = above[0]
            else:
                winner = max(valid, key=lambda x: x["vmaf_score"])
            return {
        ...
        # Only aggregate the codec that triggered this invocation.
        # Avoids writing partial winners for the other codec when both labs run concurrently.
        codecs_to_aggregate = ["libx265"] if codec_inferred == "h265" else ["libx264"]
        ...
        for res in RESOLUTIONS:
            ...
            for codec in codecs_to_aggregate:
