...
resource "aws_lambda_function" "aggregator" {
  ...
  environment {
    variables = {
      ...
      VMAF_THRESHOLD_1080P   = tostring(var.vmaf_threshold_1080p)
      VMAF_THRESHOLD_720P    = tostring(var.vmaf_threshold_720p)
      VMAF_THRESHOLD_480P    = tostring(var.vmaf_threshold_480p)
    }
  }
}
...
# s3_utils.py is bundled alongside each Lambda that imports it.
data "archive_file" "gcp_copy_zip" {
  type        = "zip"
  output_path = "/tmp/chai-q-gcp-copy.zip"
  source {
    content  = file("../orchestrator/gcp_copy_s3_to_gcs.py")
    filename = "gcp_copy_s3_to_gcs.py"
  }
  source {
    content  = file("../orchestrator/s3_utils.py")
...
locals {
  gcp_lambda_env = {
    ...
    S3_SUBTITLES_BUCKET        = aws_s3_bucket.raw_input.bucket
  }
}
